...
//  FGBase
//
//  Created by xiaoyang on 2019/12/16.
//

import Foundation
import UIKit

public protocol FYMarqueeViewCopyable {
...
extension UIView: FYMarqueeViewCopyable {
...
public enum FYMarqueeType {
    case left
    case right
    case reverse
}

public class FYMarqueeView: UIView {
    public var marqueeType: FYMarqueeType = .left
    public var contentMargin: CGFloat = 12                     //两个视图之间的间隔
    public var frameInterval: Int = 2                        //多少帧回调一次，一帧时间1/60秒
    ...
    public var contentViewFrameConfigWhenCantMarquee: ((UIView)->())?    //当contentView的内容宽度没有超过显示宽度，无需开启跑马灯效果。这个时候contentView的size，默认是调用sizeToFit之后的尺寸。如果想要特殊配置，比如让contentView的size等于FLMarqueeView，就需要在该闭包自定义配置。
...
//            }
            ...
            validContentView.frame  = self.bounds
            stopMarquee()
        }
    }
